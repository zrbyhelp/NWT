...
首页右上角“设置”提供 AI 供应商、LLM 模型、向量模型和图片模型管理。供应商按当前登录账号隔离，并按 OpenAI-compatible 协议保存 `baseUrl` 与加密后的 API Key；LLM、向量和图片模型分别只能有一个启用且供应商可用的个人默认项，模型列表可通过快捷默认图标直接切换默认项。管理员还可把 LLM、向量和图片模型设为通用，通用模型使用管理员供应商与 API Key，普通用户可见但只读；默认调用先使用当前账号个人默认，没有个人默认时使用管理员通用默认。新增或编辑模型时，前端会根据所选供应商调用服务端动作，由服务端使用当前账号已保存的 API Key 请求 `{baseUrl}/models` 拉取模型列表；若供应商不支持该接口或请求失败，仍可手动输入模型 ID。LLM 上下文窗口不在配置表单中暴露，调用时遵循模型和供应商默认策略。
